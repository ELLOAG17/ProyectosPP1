...
{
    public partial class calculadoraSC : Form
    {
        public calculadoraSC()
        {
            InitializeComponent();
        }

        private void calculadoraSC_Load(object sender, EventArgs e)
        {
...

        {
            if (txtvalores.Text != "" | operador== "√")
            {
                ...
                Clases.OperacionesSc operacion = new Clases.OperacionesSc(valor1, valor2);
                ...
                switch (operador)
                {
                    case "+":
                        ...
                        break;
                    case "-":
                        txtvalores.Text = operacion.resta().ToString();
                        break;
                    case "/":
                        txtvalores.Text = operacion.division().ToString();
                        break;

                    case "*":
                        txtvalores.Text = operacion.multiplicacion().ToString();
                        break;
                    case "%":
                        txtvalores.Text = operacion.porcentaje().ToString();
                        break;
                    case "√":
                        txtvalores.Text = operacion.raiz().ToString();
                        break;
                    default:
                        break;
                }
            ...
            {
                MessageBox.Show("Introduzca un calor");
